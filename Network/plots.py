...
def plot1(network, pace):
    ...
    ax2.set_title(
        'Relationship Between Individual and Memory for one generation')
    ...
    for i in range(len(fitness_history)):  # for each individual
              ...
              #size = cat
                if cat != 0:
                    ax2.scatter(i, memo, s=cat, marker=cercle(
                        (cat3+cat2+cat1)/cat, 1), facecolor='red')  # cat4
                    ax2.scatter(i, memo, s=cat, marker=cercle(
                        (cat2+cat1)/cat, (cat3+cat2+cat1)/cat), facecolor='orange')
                    ax2.scatter(i, memo, s=cat, marker=cercle(
        ...
        if cat != 0:
            ax2.scatter(i, memo,  s=cat, marker=cercle(
                (cat3+cat2+cat1)/cat, 1), facecolor='red')  # cat4
            ax2.scatter(i, memo,  s=cat, marker=cercle(
                (cat2+cat1)/cat, (cat3+cat2+cat1)/cat), facecolor='orange')  # cat 3
            ax2.scatter(i, memo,  s=cat, marker=cercle(
                (cat1)/cat, (cat2+cat1)/cat), facecolor='blue')
            ax2.scatter(i, memo,  s=cat, marker=cercle(
                0, (cat1)/cat), facecolor='purple')
...
def plot_transit(network, indiv):
    ...
    nx.draw_networkx_edge_labels(
        G, pos, edge_labels=dic, alpha=0.7, label_pos=0.8, font_color='red')
    plt.axis('off')
    plt.show()
    return dic
